...
const Navbar = () => {
    ...
    return (
        <header className="mx-auto px-2 md:px-10 lg:px-20  z-[1300] bg-blue-300">
            <nav className=" h-24 flex items-center justify-between relative ">
                
                <h3 onClick={handleLogoClick} className="text-pink-600 text-3xl font-bold "><HotelIcon className="text-pink-600 F" />  LIKHITH HOTEL  <HotelIcon className="text-pink-600 cursor-pointer" /> </h3>
              
                <div className="">
                    <div onClick={handleClick}>
                        <span className="md:hidden">
                            <AccountCircleIcon className="text-pink-600 cursor-pointer" />
                        </span>
                        <span className="capitalize text-pink-600 font-medium hidden md:block cursor-pointer">{isAuthenticated ? user.name : "Sign in"}</span>
                    </div>
