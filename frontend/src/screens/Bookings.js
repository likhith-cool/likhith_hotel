...
const Bookings = () => {
    ...
    return (
        <Fragment>
            ...
            <Fragment>
                {isLoading ? <Loader /> : (
                    <div className="mx-auto px-4 md:px-10 lg:px-20 xl:px-48 mt-4 bg-pink-300">
                        <h2 className="text-2xl font-medium text-center my-8 bg-pink-300">Your All Bookings</h2>
                            ...
                            <Table className="min-w-[600px]">
                                ...
                                <TableBody >
                                    {(rowsPerPage > 2 ? bookings?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : bookings)?.map((booking) => (
                                        <TableRow className="bg-yellow-300" key={booking._id} style={{height: 72.8}}>
